...
class PacketHeader
{
    public:
        ...
        virtual void get_bitstring(std::vector<int8_t> &to_fill, int8_t fill_with) = 0;
        ...
        void make_bitstring(uint32_t num_bytes, void *ptr, std::vector<int8_t> &to_fill,
                            int8_t fill_with=0);
        static void make_bitstring_header(const std::vector<std::tuple<std::string, uint32_t> > &v, 
                                   std::vector<std::string> &to_fill);
